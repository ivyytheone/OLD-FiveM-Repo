...
function JailLogin()
	...
	ESX.ShowNotification("Godmorgon!")
...
function UnJail()
	...
	ESX.ShowNotification("Du är ute ur fängelset, bete dig nu!")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Du har " .. jailTime .. " minuter kvar!")
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "Packa"

					if not v["state"] then
						PackageText = "Redan packad"
					end
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Du har redan packat här ta en annan!")
							end
						end
					end
				end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[~g~E~s~] - Öppna dörren", 0.4)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "Packar... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
	end
end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[~g~E~s~] - Lämna packet", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Fängsla person", value = "jail_closest_player" },
				{ label = "Ta ut någon ut fängelset", value = "unjail_player" }
			}
		}, 
	function(data, menu)
            	...
            	if jailTime == nil then
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Du behöver skriva något.")
						  	else
								menu3.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Det är inge som sitter i fängelset!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Fången: " .. playerArray[i].name .. " | Tid: " .. playerArray[i].jailTime .. " minuter", value = playerArray[i].identifier })
				end

				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'jail_unjail_menu',
					{
						title = "Frikänd person",
						align = "center",
						elements = elements
					},
				function(data2, menu2)
