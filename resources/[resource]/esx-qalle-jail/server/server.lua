	...
	if xPlayer["job"]["name"] == "police" then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			...
			if jailTime ~= nil then
				...
				TriggerClientEvent("esx:showNotification", src, GetPlayerName(jailPlayer) .. " Fängslade för " .. jailTime .. " minuter!")
				
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, { args = { "Fängelset",  Firstname .. " " .. Lastname .. " är nu i fängelse av anledningen: " .. args[3] }, color = { 249, 166, 0 } })
					end)
				end
			else
				TriggerClientEvent("esx:showNotification", src, "Den här tiden är ogiltig!")
			end
		else
			TriggerClientEvent("esx:showNotification", src, "Detta ID är inte online!")
		end
	else
		TriggerClientEvent("esx:showNotification", src, "Du är inte polis!")
	end
end)
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	GetRPName(targetSrc, function(Firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, { args = { "Fängelset",  Firstname .. " " .. Lastname .. " är nu i fängelse av anledningen: " .. jailReason }, color = { 249, 166, 0 } })
	end)

	TriggerClientEvent("esx:showNotification", src, GetPlayerName(targetSrc) .. " Fängslade för " .. jailTime .. " minuter!")
end)
