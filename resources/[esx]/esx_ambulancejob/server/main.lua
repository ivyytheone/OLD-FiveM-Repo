...
AddEventHandler('esx_ambulancejob:firstSpawn', function()
	...
	MySQL.Async.fetchScalar('SELECT isDead FROM users WHERE identifier=@identifier',
	{
		...
		if isDead == 1 then
			print('Spelare: ' .. GetPlayerName(_source) .. ' (' .. identifier .. ') försökte combatlogga!')
...
RegisterServerEvent('kontorr')
AddEventHandler('kontorr', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)

    xPlayer.addInventoryItem('ambcard', 1)
end)

RegisterServerEvent('rakell')
AddEventHandler('rakell', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
RegisterServerEvent('vaccin')
AddEventHandler('vaccin', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)

    xPlayer.addInventoryItem('vaccine', 5)
end)

RegisterServerEvent('tram')
AddEventHandler('tram', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)

    xPlayer.addInventoryItem('tram', 5)
end)

RegisterServerEvent('bandage')
AddEventHandler('bandage', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
