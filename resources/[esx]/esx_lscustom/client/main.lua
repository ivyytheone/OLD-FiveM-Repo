			...
			if v.modType then
				...
				if v.modType == 14 then -- HORNS
					for j = 0, 51, 1 do
						...
						if j == currentMods.modHorns then
							...
							price = math.floor(vehiclePrice * v.price / 100)
							_label = GetHornName(j) .. ' - <span style="color:green;">' .. price .. ' Sek</span>'
						end
				...
				elseif v.modType == 'plateIndex' then -- PLATES
					for j = 0, 4, 1 do
						...
						if j == currentMods.plateIndex then
							...
							price = math.floor(vehiclePrice * v.price / 100)
							_label = GetPlatesName(j) .. ' - <span style="color:green;">' .. price .. ' Sek</span>'
						end
						table.insert(elements, {label = _label, modType = k, modNum = j})
					end
				elseif v.modType == 22 then -- NEON
					...
					if currentMods.modXenon then
						...
						price = math.floor(vehiclePrice * v.price / 100)
						_label = _U('neon') .. ' - <span style="color:green;">' .. price .. ' Sek</span>'
					end
				...
				elseif v.modType == 'neonColor' or v.modType == 'tyreSmokeColor' then -- NEON & SMOKE COLOR
					...
					for i=1, #neons, 1 do
						table.insert(elements, {
							label = '<span style="color:rgb(' .. neons[i].r .. ',' .. neons[i].g .. ',' .. neons[i].b .. ');">' .. neons[i].label .. ' - <span style="color:green;"' .. price .. ' Sek</span>',
							modType = k,
				...
				elseif v.modType == 'color1' or v.modType == 'color2' or v.modType == 'pearlescentColor' or v.modType == 'wheelColor' then -- RESPRAYS
					...
					for j = 1, #colors, 1 do
						local _label = ''
						price = math.floor(vehiclePrice * v.price / 100)
						_label = colors[j].label .. ' - <span style="color:green;">' .. price .. ' Sek</span>'
				...
				elseif v.modType == 'windowTint' then -- WINDOWS TINT
					for j = 1, 5, 1 do
						...
						if j == currentMods.modHorns then
							...
							price = math.floor(vehiclePrice * v.price / 100)
							_label = GetWindowName(j) .. ' - <span style="color:green;">' .. price .. ' Sek</span>'
						end
						table.insert(elements, {label = _label, modType = k, modNum = j})
					end
					...
					for j = 0, modCount, 1 do
						...
						if modName then
							...
							if j == currentMods.modFrontWheels then
								...
								price = math.floor(vehiclePrice * v.price / 100)
								_label = GetLabelText(modName) .. ' - <span style="color:green;">' .. price .. ' Sek</span>'
							end
				...
				elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
					...
					for j = 0, modCount, 1 do
						...
						if j == currentMods[k] then
							...
							_label = _U('level', j+1) .. ' - <span style="color:green;">' .. price .. ' Sek</span>'
						end
						table.insert(elements, {label = _label, modType = k, modNum = j})
				...
				elseif v.modType == 17 then -- TURBO
					...
					if currentMods[k] then
						...
						_label = 'Turbo - <span style="color:green;">' .. math.floor(vehiclePrice * v.price[1] / 100) .. ' Sek</span>'
					end
					table.insert(elements, {label = _label, modType = k, modNum = true})
				else
