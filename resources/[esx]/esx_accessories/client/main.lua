		...
		if hasAccessory then
				...
				TriggerEvent('skinchanger:getSkin', function(skin)
					...
					if skin[_accessory .. '_1'] == mAccessory then
						ESX.Streaming.RequestAnimDict('mp_masks@on_foot', function()
							TaskPlayAnim(playerPed, 'mp_masks@on_foot', 'put_on_mask', 2.0, -1, 800, 49, 0, 0, 0, 0)
							Citizen.Wait(700)
						end)
						...
						ESX.Streaming.RequestAnimDict('missfbi4', function()
							TaskPlayAnim(playerPed, 'missfbi4', 'takeoff_mask', 2.0, -1, -1, 49, 0, 0, 0, 0)
							Citizen.Wait(1400)
						end)
						ClearPedSecondaryTask(playerPed)
						SetUnsetAccessory('Helmet')
			...
			if accessory == 'Mask' then
				TriggerEvent('skinchanger:getSkin', function(skin)
					...
					if skin[_accessory .. '_1'] == mAccessory then
						ESX.Streaming.RequestAnimDict('mp_masks@on_foot', function()
							TaskPlayAnim(playerPed, 'mp_masks@on_foot', 'put_on_mask', 2.0, -1, -1, 49, 0, 0, 0, 0)
							Citizen.Wait(500)
						end)
						ClearPedSecondaryTask(playerPed)
						...
						ESX.Streaming.RequestAnimDict('misscommon@std_take_off_masks', function()
							TaskPlayAnim(playerPed, 'misscommon@std_take_off_masks', 'take_off_mask_ps', 2.0, -1, -1, 49, 0, 0, 0, 0)
							Citizen.Wait(1400)
						end)
						ClearPedSecondaryTask(playerPed)
						SetUnsetAccessory('Mask')
				...
				TriggerEvent('skinchanger:getSkin', function(skin)
					...
					if skin[_accessory .. '_1'] == mAccessory then
						ESX.Streaming.RequestAnimDict('clothingspecs', function()
							TaskPlayAnim(playerPed, 'clothingspecs', 'take_off', 2.0, -1, 800, 49, 0, 0, 0, 0)
							Citizen.Wait(2500)
		...
		for k,v in pairs(Config.Zones) do
			for i = 1, #v.Pos, 1 do
				if(Config.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos[i].x, v.Pos[i].y, v.Pos[i].z, true) < 10) then
					ESX.Game.Utils.DrawText3D(v.Pos[i] - vector3(0.0, 0.0, -0.9), '[~g~E~s~] Öppna menyn')
				end
			end
		end
	end
