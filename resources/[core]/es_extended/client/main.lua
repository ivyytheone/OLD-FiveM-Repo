	...
	while true do
		...
		for k,v in pairs(Pickups) do
			...
			if distance <= 5.0 then
				DrawText3D(v.coords.x, v.coords.y, v.coords.z - 0.5, v.label, 0.2)
			end

			if distance <= 1.0 and not v.inRange and not IsPedSittingInAnyVehicle(playerPed) then
				ESX.ShowHelpNotification('~INPUT_CONTEXT~ Plocka upp ' .. v.label)
				if IsControlJustPressed(0, Keys["E"]) then
					RequestAnimDict("missheist_agency2aig_13")
					while not HasAnimDictLoaded("missheist_agency2aig_13") do
						Citizen.Wait(0)
					end
					TaskPlayAnim(GetPlayerPed(-1), "missheist_agency2aig_13" ,"pickup_briefcase" ,8.0, -8.0, 2000, 0, 0, false, false, false )
