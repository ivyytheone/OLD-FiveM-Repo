Citizen.CreateThread(function() 
    while true do 
        ...
        if IsControlJustReleased(0, 167) and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == "taxi" then  
            OpenMenu()
        end
    end
end)
...
function OpenMenu()  
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'taxi', {
        title = 'Taxi', 
        align = 'top-right', 
        elements = mElements
    }, function(data, menu) 
        ...
        if action == "bossMenu" then 
            ESX.UI.Menu.Open("default", GetCurrentResourceName(), "bossMenu", {
                ...
                if Value == "send_invoice_to_company" then  
                    ...
                    exports.billing:showJobInvoices("taxi")
                end
