$(function () {
    ...
    let VehicleMenu = ["Sätt på / av bälte", "Dörrar", "Fönsterrutor", "Handsfack"];
    ...
    document.onkeyup = function (data) {
        if (data.which == 27 || data.which == 90) 
        {
            ...
            clearObjectData();
            clearContextmenu();
            $.post('https://eyrp_contextmenu/exit', JSON.stringify({}));
            return
        }
    };
    ...
    document.addEventListener("click", (e) => {
        ...
        {
            case "use":
                $.post('https://eyrp_contextmenu/use', JSON.stringify({})); //General options
                break;
            case "examine":
                $.post('https://eyrp_contextmenu/examine', JSON.stringify({})); //General options
                break;
            case "close":
                $.post('https://eyrp_contextmenu/exit', JSON.stringify({})); //General options
                break;
        }
        clearObjectData();
    })
    ...
    {
        $.post('https://eyrp_contextmenu/doors', JSON.stringify({index: doorIndex}));
        return
    }
    //Context menu -- right click
    window.addEventListener("contextmenu",function(event){
        event.preventDefault();
        $.post('https://eyrp_contextmenu/rightclick', JSON.stringify({}));
    ...
    {
        createMenuItem("Använd", "use");
        createMenuItem("Individåtgärder", "interact-menu");
        createSubMenuItems(policeMenu, "interact-sub-menu")
        createMenuItem("Animationer", "anim");
        createMenuItem("Accessoarer", "accessories");
        checkIfVehicle();
        createMenuItem("Stäng", "close");
    ...
    {
        if(objectType == 2) {
            createMenuItem("Fordonsmeny", "vehicle-menu"); //Creates the Initial Menu option.
            createSubMenu("vehicle-sub-menu","vehicle-menu"); //Adds the ID handles to a Sub menu element
            createSubMenuItems(VehicleMenu, "vehicle-sub-menu"); //Adds the Menu options to the specific Sub menu.
        }
    }
