...
--index.js eyrp_contextmenu event click (right click) -> eyrp_contextmenu-c.lua toggle the raytrace
...
--Based on menu element selected -> eyrp_contextmenu-c.lua (e.g. RegisterNUICallback("doors", function(data), sets the state of the vehicle doors)
...
--Sends over the object data(type, job, admin, isplayer) to the Javascript to determine what menus should be added to the eyrp_contextmenu on creation.
...
--These are called from the Javascript via .. $.post('http://eyrp_contextmenu/rigthclick', JSON.stringify({}));
